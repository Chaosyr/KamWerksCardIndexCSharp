...
{
	/// <summary>
	/// Handles the Fancy Formatting for CTI Sigils via Commaands.
	/// </summary>
	/// <remarks>This code is provided by Creator/Chaosyr/SaxbyMod/The Stoat Lord.</remarks>
	public class Fancy_CTI_Sigil
	{
		/// <summary>
		/// Runs CTI Sigil in Fancy Mode.
		/// </summary>
		...
		public async static Task<(MemoryStream image, string takeout)> CTIFromCard(string set, string name, string Token)
		{
			...
			var ctiProperties = await CTI_Properties.FetchPageProperties(name, set, "Sigil");
			...
			image.Mutate(img =>
				{
					img.DrawText(optionsTitle, ctiProperties[1], Color.White);
					img.DrawText(optionsNormal, ctiProperties[2].Replace("[Token]", Token), Color.White);
					img.DrawText(optionsSubSubText, ctiProperties[3], Color.White);
					sigil.Mutate(img =>
						{
							img.Invert();
			...
			await image.SaveAsync(stream, PngFormat.Instance);
			stream.Position = 0;
			var id = NotionEnd.CtiSigils.GetValueOrDefault(name);
			string cleanPageId = id.Replace("-", "");
			var output = $"Provided content for this embed found from; <https://inscryption-pvp-wiki.notion.site/{cleanPageId}>";
			var takeout = output;
